...
from config import *
from runner.loss import ContrastiveLoss
...
def run(model, ratio, step, thold):
    global trial
    trial += 1
    best = 0
    ...
    train_feature, train_label = transformation.make_siamese_batch(train)
    test_feature, test_label = transformation.make_siamese_batch(test)
    ...
    for i in range(step + 1):
        ...
        one, two = x[:, 0, :, :], x[:, 1, :, :]
        one, two = model.forward(one, two)
        y = train_label.to(device).float()

        opt.zero_grad()
        error = criterion(one, two, y)
        error.backward()
        opt.step()
        ...
        if i % record_per_step == 0:
            ...
            pred = [1 if k.item() * 100 > thold else 0 for k in F.pairwise_distance(one, two)]
